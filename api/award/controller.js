...
const { AwardModel, MetaModel }             = require('@database');
...
const list = async (req, res, next) => {
    try {
        ...
        delete query.limit;
        let record = {};
        record.awardList = await AwardModel.find(query,{createdBy:0,updatedBy:0,createdAt:0,updatedAt:0}).sort({sortOrder: 1}).limit(limit).skip(pagination*limit).populate('file', 'name original path thumbnail smallFile');
     
        record.meta = await MetaModel.find({active:true,link:'award'},{createdBy:0,updatedBy:0,createdAt:0,updatedAt:0}).populate('file', 'name original path thumbnail smallFile');
